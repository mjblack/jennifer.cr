...
describe Jennifer::Adapter::DBColorizedFormatter do
  ...
  describe "#run" do
    it "formats an entry" do
      ...
      io.to_s
        .should match(/^\e\[38;2;17;120;100mdb\e\[(0|39)m: #{Regex.escape(metadata[:time].to_s)} ms \e\[36m#{Regex.escape(metadata[:query])}\e\[(0|39)m \| \e\[33m#{Regex.escape(metadata[:args])}\e\[(0|39)m$/)
    end
  end
end
